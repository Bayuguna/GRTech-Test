...
import EditAccess from "./FormAccess/EditAccess.vue";
import CreateAccess from "./FormAccess/CreateAccess.vue";

const accesses = ref([]); // Data storage
...
const fetchCompanies = async () => {
    try {
        const response = await axios.get("/settings/accesses/data");
...
// Delete Access Data
const deleteAccess = async (employee_uuid: string) => {
    try {
        await axios.delete("/settings/accesses/delete/" + employee_uuid);
        fetchCompanies();
    } catch (error) {
...
<template>
    ...
    <div v-if="editModal">
        <a-modal v-model:open="editModal" title="Edit Access" :footer="null">
            <EditAccess :uuid="employeeUUID" />
        </a-modal>
    </div>

    <div v-if="addModal">
        <a-modal v-model:open="addModal" title="Create Access" :footer="null">
            <CreateAccess />
        </a-modal>
    </div>
</template>
